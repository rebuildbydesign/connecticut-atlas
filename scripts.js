...
let CT_CONGRESS_GEOJSON = null;
let CT_HOUSE_GEOJSON = null;
let CT_SENATE_GEOJSON = null;

// INITIALIZE MAP
var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/mapbox/light-v11',
    center: [-72.7554, 41.58], // CENTERED ON CONNECTICUT
    zoom: 8.3,
    minZoom: 6.4
});
...
function addLayers() {
    map.addSource('connecticutFema', {
        type: 'geojson',
        data: 'data/CT_FEMA_County.geojson'
    });

    map.addLayer({
        'id': 'femaDisasters',
        'type': 'fill',
        'source': 'connecticutFema',
        'paint': {
            'fill-color': [
                'match',
...
function addCongressionalLayers() {
  fetch('data/CT_Congress.geojson')
    .then(r => r.json())
    .then(data => {
      MA_CONGRESS_GEOJSON = data;
      map.addSource('ctCongress', { type: 'geojson', data });
...
function addHouseLayers() {
  fetch('data/CT_House.geojson')
    .then(r => r.json())
    .then(data => {
      MA_HOUSE_GEOJSON = data;
      map.addSource('ctHouse', { type: 'geojson', data });
...
function addSenateLayers() {
  fetch('data/CT_Senate.geojson')
    .then(r => r.json())
    .then(data => {
      MA_SENATE_GEOJSON = data;
      map.addSource('ctSenate', { type: 'geojson', data });
    ...
    return `
      <div style="color:#222; font-family:inherit;">
        ...
        <ul style="list-style:none; padding:0; margin:0 0 0.9em 0;">
          <li style="margin-bottom: 3px;"><strong>U.S. Senate:</strong> Chris Murphy (D), Richard Blumenthal (D)</li>
